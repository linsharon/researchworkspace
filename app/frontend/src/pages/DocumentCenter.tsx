...
export default function DocumentCenter() {
  ...
  const loadDocuments = async (nextOffset = offset) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(isZh ? "加载文档失败" : isZh ? "加载文档失败" : isZh ? "加载文档失败" : "Failed to load documents");
    } finally {
      setLoading(false);
    }
  };
  ...
  const loadRecycleBin = async () => {
    ...
    try {
      ...
      console.error(error);
      toast.error(isZh ? "加载回收站失败" : isZh ? "加载回收站失败" : isZh ? "加载回收站失败" : "Failed to load recycle bin");
    } finally {
  ...
  const loadVersions = async (doc: DocumentItem) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(isZh ? "加载版本失败" : isZh ? "加载版本失败" : isZh ? "加载版本失败" : "Failed to load versions");
      setVersions([]);
    } finally {
  ...
  const loadShares = async (doc: DocumentItem) => {
    ...
    try {
      ...
      console.error(error);
      toast.error(isZh ? "加载共享失败" : isZh ? "加载共享失败" : isZh ? "加载共享失败" : "Failed to load shares");
      setShares([]);
    } finally {
  ...
  const handleGrantShare = async () => {
    if (!shareDoc || !selectedCandidate?.id) {
      toast.error(isZh ? "请选择要授权的用户" : isZh ? "请选择要授权的用户" : isZh ? "请选择要授权的用户" : "Please select a user to grant access");
      return;
    }
    try {
      ...
      toast.success(isZh ? "授权成功" : isZh ? "授权成功" : isZh ? "授权成功" : "Access granted");
      setShareQuery("");
      setShareCandidates([]);
      setSelectedCandidate(null);
  ...
  const handleRevokeShare = async (granteeUserId: string) => {
    ...
    try {
      ...
      toast.success(isZh ? "权限被撤销" : isZh ? "权限被撤销" : isZh ? "权限被撤销" : "Access revoked");
      await loadShares(shareDoc);
    } catch (error: unknown) {
      const detail = (error as { response?: { data?: { detail?: string } } })?.response?.data?.detail;
  ...
  const handleCreateDocument = async () => {
    ...
    if (!title) {
      toast.error(isZh ? "文档标题是必须的" : isZh ? "文档标题是必须的" : isZh ? "文档标题是必须的" : "Document title is required");
      return;
    }
    ...
    try {
      if (newPermission === "team" && !newProjectId.trim()) {
        toast.error(isZh ? "团队文档需要项目ID" : isZh ? "团队文档需要项目ID" : isZh ? "团队文档需要项目ID" : "Team document requires a project ID");
        return;
      }
      ...
      toast.success(isZh ? "文档创建成功" : isZh ? "文档创建成功" : isZh ? "文档创建成功" : "Document created");
  ...
  const handleDelete = async (doc: DocumentItem) => {
    try {
      ...
      toast.success(isZh ? "已移至回收站" : isZh ? "已移至回收站" : isZh ? "已移至回收站" : "Moved to recycle bin");
      ...
      console.error(error);
      toast.error(isZh ? "删除文档失败" : isZh ? "删除文档失败" : isZh ? "删除文档失败" : "Failed to delete document");
    }
  };
  ...
  const handleRestore = async (doc: DocumentItem) => {
    try {
      ...
      toast.success(isZh ? "文档已恢复" : isZh ? "文档已恢复" : isZh ? "文档已恢复" : "Document restored");
      await loadDocuments(0);
      await loadRecycleBin();
    } catch (error) {
      console.error(error);
      toast.error(isZh ? "恢复文档失败" : isZh ? "恢复文档失败" : isZh ? "恢复文档失败" : "Failed to restore document");
    }
  };
  ...
  const handleStatusChange = async (doc: DocumentItem, status: DocumentStatus) => {
    try {
      ...
      toast.success(isZh ? "状态已更新" : isZh ? "状态已更新" : isZh ? "状态已更新" : "Status updated");
  ...
  const handleUploadFile = async (doc: DocumentItem, file: File) => {
    ...
    try {
      ...
      toast.success(isZh ? "文件上传成功并追加版本" : isZh ? "文件上传成功并追加版本" : isZh ? "文件上传成功并追加版本" : "File uploaded and version appended");
      if (selectedDoc?.id === doc.id) {
  ...
  const handleDownload = async (params: {
    ...
    try {
      ...
      if (!response.download_url) {
        toast.error(isZh ? "下载链接不可用" : isZh ? "下载链接不可用" : isZh ? "下载链接不可用" : "Download URL is unavailable");
        return;
      }
        ...
        <Card className="border-slate-700/50">
          ...
          <CardContent className="space-y-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              <Input value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder={isZh ? "标题" : isZh ? "标题" : isZh ? "标题" : "Title"} />
              <Select value={newPermission} onValueChange={(v) => setNewPermission(v as DocumentPermission)}>
                <SelectTrigger>
                  <SelectValue placeholder={isZh ? "权限" : isZh ? "权限" : isZh ? "权限" : isZh ? "权限" : isZh ? "权限" : "Permission"} />
                </SelectTrigger>
                <SelectContent>
            ...
            <Input value={newProjectId} onChange={(e) => setNewProjectId(e.target.value)} placeholder={isZh ? "项目ID（团队权限需要）" : isZh ? "项目ID（团队权限需要）" : isZh ? "项目ID（团队权限需要）" : "Project ID (required for team permission)"} />
            <Input value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder={isZh ? "描述" : isZh ? "描述" : isZh ? "描述" : "Description"} />
            <Input value={newTags} onChange={(e) => setNewTags(e.target.value)} placeholder={isZh ? "标签（逗号分隔）" : isZh ? "标签（逗号分隔）" : isZh ? "标签（逗号分隔）" : "Tags (comma separated)"} />
              ...
              <CardContent className="space-y-3">
                <div className="flex gap-2 flex-wrap">
                  <div className="relative flex-1 min-w-[240px]">
                    ...
                    <Input
                      ...
                      placeholder={isZh ? "按标题/描述搜索" : isZh ? "按标题/描述搜索" : isZh ? "按标题/描述搜索" : "Search by title/description"}
                    />
                  </div>
                  <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as DocumentStatus | "all") }>
                    <SelectTrigger className="w-[180px]">
                      <SelectValue placeholder={isZh ? "状态" : isZh ? "状态" : isZh ? "状态" : "Status"} />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div className="flex gap-2 flex-wrap">
                  <Input
                    ...
                    placeholder={isZh ? "按标签过滤" : isZh ? "按标签过滤" : isZh ? "按标签过滤" : "Filter by tag"}
                  />
                  <Input
                    className="w-[180px]"
                    type="date"
                ...
                <>
                  ...
                  <div className="flex gap-1 flex-wrap">
                    <Input
                      ...
                      placeholder={isZh ? "按邮箱/姓名搜索用户" : isZh ? "按电子邮件/姓名搜索用户" : isZh ? "按邮箱/姓名搜索用户" : "Search user by email/name"}
                      value={shareQuery}
                      onChange={(e) => {
