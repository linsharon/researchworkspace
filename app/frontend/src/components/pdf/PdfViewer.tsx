...
export default function PdfViewer({
  ...
  const { lang } = useI18n();
  const isZh = lang === "zh";
  const resolvedTitle = title || (isZh ? "PDF阅读器" : isZh ? "PDF阅读器" : isZh ? "PDF阅读器" : "PDF Viewer");
  ...
  const handleHighlight = async () => {
    ...
    if (paperId) {
      try {
        ...
        toast.success(isZh ? "高亮保存" : isZh ? "高亮保存" : isZh ? "高亮保存" : "Highlight saved", {
          description: "Added to Highlights tab.",
        });
      } catch (error) {
        console.error("Failed to save highlight:", error);
        toast.error(isZh ? "保存失败" : isZh ? "保存失败" : isZh ? "保存失败" : "Save failed", {
  ...
  const handleSaveConcept = async () => {
    ...
    if (typeof window !== "undefined") {
      ...
      toast.success(isZh ? "关键词已保存" : isZh ? "关键词已保存" : isZh ? "关键词已保存" : "Keyword saved", {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              ...
              <Input
                ...
                placeholder={isZh ? "输入关键词名称..." : isZh ? "输入关键词名称..." : isZh ? "输入关键词名称..." : "Enter keyword name..."}
                className="text-sm"
            ...
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                placeholder={isZh ? "在您的研究语境中，这个关键词是什么意思？添加细节、定义或注释..." : isZh ? "在您的研究语境中，这个关键词是什么意思？添加细节、定义或注释..." : isZh ? "在您的研究语境中，这个关键词是什么意思？添加细节、定义或注释..." : "What does this keyword mean in your research context? Add details, definitions, or notes..."}
                rows={3}
